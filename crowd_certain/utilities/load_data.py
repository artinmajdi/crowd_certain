...
class LOAD_CIFAR100():
    ...
    def load(self, data_mode=['train', 'valid', 'test'], label_type='merged', approach='manual'):
        ...
        if approach == 'manual':
            self.load_cifar100_manually(
                data_modes=data_mode, label_type=label_type)
            self.get_iterators(
                data_modes=['train'], be_augmented=True, Keras=True, TF=False, batch_size=128)
            self.get_iterators(
                data_modes=['valid'], be_augmented=False, Keras=True, TF=False, batch_size=128)
