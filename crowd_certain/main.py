...
def parse_args():
    ...
    single_parser.add_argument("--dataset", "-d", type=str, required=True,
                              choices=[name.value for name in DatasetNames],
                              help="Dataset name to use for simulation")
    single_parser.add_argument("--workers-min", type=int, default=3,
                              help="Minimum number of workers (default: 3)")
    single_parser.add_argument("--workers-max", type=int, default=8,
                              help="Maximum number of workers (default: 8)")
    single_parser.add_argument("--quality-min", type=float, default=0.4,
                              help="Minimum worker quality (default: 0.4)")
    single_parser.add_argument("--quality-max", type=float, default=1.0,
                              help="Maximum worker quality (default: 1.0)")
    single_parser.add_argument("--seeds", type=int, default=3,
                              help="Number of random seeds (default: 3)")
    single_parser.add_argument("--read-mode", type=str, default="AUTO",
                              choices=[mode.value for mode in ReadMode],
                              help="Read mode for datasets (default: AUTO)")
    single_parser.add_argument("--output-dir", type=str, default=None,
                              help="Directory to save results (default: None, results not saved)")

    # Run simulation on all datasets
    all_parser = subparsers.add_parser("all", help="Run simulation on all datasets")
    all_parser.add_argument("--workers-min", type=int, default=3,
                           help="Minimum number of workers (default: 3)")
    all_parser.add_argument("--workers-max", type=int, default=8,
                           help="Maximum number of workers (default: 8)")
    all_parser.add_argument("--quality-min", type=float, default=0.4,
                           help="Minimum worker quality (default: 0.4)")
    all_parser.add_argument("--quality-max", type=float, default=1.0,
                           help="Maximum worker quality (default: 1.0)")
    all_parser.add_argument("--seeds", type=int, default=3,
                           help="Number of random seeds (default: 3)")
    all_parser.add_argument("--read-mode", type=str, default="AUTO",
                           choices=[mode.value for mode in ReadMode],
                           help="Read mode for datasets (default: AUTO)")
...
def run_single_dataset(args):
    ...
    dataset_name = next((name for name in DatasetNames if name.value == args.dataset), None)
