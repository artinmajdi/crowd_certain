...
class ReliabilityCalculator:
	...
	"""
	def __init__(self, config: 'Settings', n_workers: int, seed: int, data: dict[str, pd.DataFrame], feature_columns: list[str]):
		self.config = config
		self.seed = seed
		self.random_state = np.random.default_rng(self.seed)
		self.n_workers = n_workers
		self.classifier_training = ClassifierTraining(config=config)
		self.data = data
	...
	def calculate_uncertainties(self, df: pd.DataFrame) -> pd.DataFrame:
		...
		for tech in self.config.technique.uncertainty_techniques:
			...
			if tech is params.UncertaintyTechniques.STD:
				...
				confidence_interval = df.apply(lambda row: scipy.stats.norm.interval(0.95, loc=np.mean(row), scale=scipy.stats.sem(row)) if np.std(row) > 0 else (np.nan, np.nan),axis=1).apply(pd.Series)

				# Calculate the width of the confidence interval (uncertainty score)
				weight = confidence_interval[1] - confidence_interval[0]

				# Optional: Normalize by the mean or another relevant value,
				# For example, normalize by the midpoint of the interval
				# midpoint = (confidence_interval[1] + confidence_interval[0]) / 2
	...
	def assigning_strengths_randomly_to_each_worker(self) -> pd.DataFrame:
		"""
		...
		workers_strength_array = self.random_state.uniform(low=self.config.simulation.low_dis, high=self.config.simulation.high_dis, size=self.n_workers)

		return pd.DataFrame({'workers_strength': workers_strength_array}, index=workers_names)


	def initialize_variables(self, workers_strength: pd.DataFrame) -> tuple[dict[str, dict[str, pd.DataFrame]], dict[str, pd.DataFrame], dict[str, pd.DataFrame]]:
	...
	def update_uncertainties(self, LB: str, classifier, predicted_labels_all_sims: dict[str, pd.DataFrame], uncertainties: dict[str, pd.DataFrame]) -> tuple[dict[str, pd.DataFrame], dict[str, pd.DataFrame]]:
		"""
		...
		classifier : sklearn estimator
			The trained classifier used for predictions.
		predicted_labels_all_sims : dict[str, pd.DataFrame]
			Dictionary containing prediction results for all simulations.
		uncertainties : dict[str, pd.DataFrame]
			Dictionary to store calculated uncertainty values.

		Returns
		-------
		tuple[dict[str, pd.DataFrame], dict[str, pd.DataFrame]]
			Updated predicted_labels_all_sims and uncertainties dictionaries.
		...
		return predicted_labels_all_sims, uncertainties


	def getting_noisy_manual_labels_for_each_worker(self, truth_array: np.ndarray, l_strength: float) -> np.ndarray:
		"""
		...
		true_label_assignment_prob = self.random_state.random(num_samples)
	...
	def update_noisy_labels(self, LB: str, predicted_labels_all_sims: dict[str, pd.DataFrame], truth: dict[str, pd.DataFrame], workers_strength: pd.DataFrame) -> tuple[dict[str, pd.DataFrame], dict[str, pd.DataFrame]]:
		"""
		...
		for mode in ['train', 'test']:
			...
			truth[mode][LB] = self.getting_noisy_manual_labels_for_each_worker( truth_array=self.data[mode].true.values, l_strength=workers_strength.loc[LB, 'workers_strength'] )

		return truth, predicted_labels_all_sims


	def update_predicted_labels_all_sims(self, LB: str, sim_num: int, predicted_labels_all_sims: dict[str, pd.DataFrame], truth: dict[str, pd.DataFrame]) -> tuple[dict[str, pd.DataFrame], dict[str, pd.DataFrame]]:
		"""
		...
		-------
		tuple[dict[str, pd.DataFrame], dict[str, pd.DataFrame]]
			Updated predicted_labels_all_sims dictionary and the trained classifier.
		...
		return predicted_labels_all_sims, classifier


	def swap_axes(self, predicted_labels_all_sims: dict[str, dict[str, pd.DataFrame]]) -> dict[str, dict[str, pd.DataFrame]]:
		"""
	...
	def looping_over_all_workers(self, workers_strength: pd.DataFrame) -> tuple[dict[str, pd.DataFrame], dict[str, pd.DataFrame], dict[str, dict[str, pd.DataFrame]]]:
		"""
		...
		predicted_labels_all_sims, truth, uncertainties = self.initialize_variables(workers_strength=workers_strength)

		for LB_index, LB in enumerate(workers_strength.index):

			truth, predicted_labels_all_sims = self.update_noisy_labels(LB=LB, predicted_labels_all_sims=predicted_labels_all_sims, truth=truth, workers_strength=workers_strength)

			for sim_num in range(self.classifier_training.n_simulations):
				predicted_labels_all_sims, classifier = self.update_predicted_labels_all_sims(LB=LB, sim_num=sim_num, predicted_labels_all_sims=predicted_labels_all_sims, truth=truth)

			predicted_labels_all_sims, uncertainties = self.update_uncertainties(LB=LB, classifier=classifier, predicted_labels_all_sims=predicted_labels_all_sims, uncertainties=uncertainties)

		preds = self.swap_axes(predicted_labels_all_sims=predicted_labels_all_sims)

		return truth, uncertainties, preds


	def adding_accuracy_for_each_worker(self, workers_strength: pd.DataFrame, preds: dict[str, dict[str, pd.DataFrame]], truth: dict[str, pd.DataFrame]) -> pd.DataFrame:
		"""
	...
	def calculate_probs_and_uncertainties(self) -> tuple[dict[str, dict[str, pd.DataFrame]], dict[str, pd.DataFrame], dict[str, pd.DataFrame], pd.DataFrame]:
		"""
		...
		Returns:
		-------
		tuple[dict[str, dict[str, pd.DataFrame]], dict[str, pd.DataFrame], dict[str, pd.DataFrame], pd.DataFrame]
			A tuple containing:
		...
		workers_strength = self.assigning_strengths_randomly_to_each_worker()

		truth, uncertainties, preds = self.looping_over_all_workers(workers_strength=workers_strength)

		workers_strength = self.adding_accuracy_for_each_worker(workers_strength, preds, truth)
	...
	def calculate_consistency(self, uncertainty: Union[pd.DataFrame, pd.Series, np.ndarray]) -> pd.DataFrame:
		...
		if isinstance(uncertainty, pd.DataFrame):
			...
			# TODO: delete this multistructure and only preserve either pd.Series or pd.DataFrame (after figuring out how uncertainty calculation works)
