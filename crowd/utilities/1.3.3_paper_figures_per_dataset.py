...
np.random.seed(0)
# data, feature_columns = load_data.aim1_3_read_download_UCI_database(dataset_name=dataset, mode='read')
...
mlflow_setup.log_artifact( data=outputs, path=f'outputs/outputs_{dataset}.pkl', artifact_path='' )
...
mlflow_setup.log_artifact( data=A13R.accuracy_seeds, path=f'outputs/accuracy_seeds_{dataset}.pkl', artifact_path='' )


""" 2.3 Final results - average over all seeds """
A13R.avg_accuracy_over_all_seeds(run_stacking=False)
mlflow_setup.log_artifact( data=A13R.accuracy, path=f'outputs/accuracy_{dataset}.pkl', artifact_path='' )
